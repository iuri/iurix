ad_library {
    ...
    @cvs-id $Id: ecommerce-procs.tcl,v 1.36 2010/10/15 05:25:32 torbenb Exp $
...
ad_proc ec_system_owner {} {
    @return ec system owner, defaulting to ad_system_owner
     ...
     return [util_memoize [list ec_system_owner_mem] [ec_cache_refresh]]
}
...
ad_proc -public ec_id {} {
    ...
    to the first site if more than one
     ...
     return [util_memoize [list ec_id_mem] 300]
}
...
ad_proc -public ec_url {
    ...
    @return The url (without protocol or port) of the ecommerce mountpoint if it exists, 0 otherwise.
     ...
     return [util_memoize [list ec_url_mem] [ec_cache_refresh]]
}
...
ad_proc -private ec_convert_path_mem {} {} {
    # Stopped HERE Error related ec_id parsing commas to Qyery ad_parameter " "

    ns_log Notice "Running script ec_convert_path_mem"
    return [util_memoize "parameter::get -package_id [ec_id] -parameter ImageMagickPath -default /usr/bin/convert" [ec_cache_refresh]]
...
ad_proc -private ec_product_directory_mem {} {
    ...
    return [parameter::get -package_id [ec_id] -parameter ProductDataDirectory -default "product/"]
}
...
ad_proc ec_shipping_cost_summary { base_shipping_cost default_shipping_per_item weight_shipping_cost add_exp_base_shipping_cost add_exp_amount_per_item add_exp_amount_by_weight } { returns cost summary } {
    ...
    set currency [parameter::get -package_id [ec_id] -parameter Currency -default ecommerce ]
	...
	append shipping_summary "the per-item-cost is equal to [ec_pretty_pure_price $weight_shipping_cost $currency] times its weight in [parameter::get -package_id [ec_id] -parameter WeightUnits], unless the \"Shipping Price\" has been set for that product. A \"Shipping Price - Additional\" amount is added for each additional unit of the same product ordered).  "
    }
	...
	if { ![empty_string_p $add_exp_amount_by_weight] && $add_exp_amount_by_weight != 0 } {
	    append express_part_of_shipping_summary "Express Shipping adds [ec_pretty_pure_price $add_exp_amount_by_weight $currency] per [parameter::get -package_id [ec_id] -parameter WeightUnits] of each item ordered, in addition to the amount charged for Regular Shipping.  "
	}
    }
...
ad_proc ec_mailing_list_link_for_a_product { product_id } { 
    ...
    if { ![empty_string_p $category_id] || ![empty_string_p $subcategory_id] || ![empty_string_p $subsubcategory_id] } {
	set url [export_vars -base "[ec_url]mailing-list-add" {category_id subcategory_id subsubcategory_id}]
        return "<a href=\"$url\">#ecommerce.Subscribe_to# <b>[ec_full_categorization_display $category_id $subcategory_id $subsubcategory_id]</b> #ecommerce.announcements#</a>."
    } else { 
        return ""
    }
}
...
ad_proc ec_display_rating { rating } { 
	...
	for { set counter 0 } { $counter < $n_full_stars } { incr counter } {
	    append rating_to_print "<img width=[lindex $full_star_gif_size 0] height=[lindex $full_star_gif_size 1] src=\"/favicon.png\" alt=\"Star\">"
	}
...
ad_proc ec_customer_comments { product_id {comments_sort_by ""} {prev_page_url ""} {prev_args_list ""} } { returns customer comments } {
    ...
    if { [parameter::get -package_id [ec_id] -parameter ProductCommentsAllowP] == 0 } {
	return ""
    }
    ...
    if { [parameter::get -package_id [ec_id] -parameter ProductCommentsNeedApprovalP -default 0 -boolean] } {
	...
	Number of reviews: [db_string n_reviews_select "
            select count(*) from ec_product_comments where product_id = :product_id and (approved_p='t' [ec_decode [parameter::get -package_id [ec_id] -parameter ProductCommentsNeedApprovalP -default 0 -boolean] "0" "or approved_p is null" ""])
        "] ($sort_blurb)</p>
	$comments_to_print
	<p><a href=\"[ec_url]review-submit?[export_url_vars product_id]\">Review this item</a>.</p>"
    } else {
        append to_return "<p><a href=\"[ec_url]review-submit?[export_url_vars product_id]\">#ecommerce.Be_the_first_lt#</a></p>\n"
    }
...
ad_proc ec_add_to_cart_link {
    product_id
    {add_to_cart_button_text "#ecommerce.To_buy#"}
    ...
    ns_log Notice "Running ad_proc ec_add_to_cart... "
    ...
    if { $no_shipping_avail_p == "t" } {
        append warnings "#ecommerce.Warnings_lt#"
    }
...
<img alt=\"\" border=\"0\" src=\"https://www.paypal.com/en_US/i/scr/pixel.gif\" width=\"1\" height=\"1\">
	...
	[export_form_vars product_id]
        [export_form_vars access_token]
        [export_form_vars expires_in]
        [export_form_vars app_id]
 ...
 <input type=submit value=\"[ad_quotehtml $add_to_cart_button_text]\"><br>
        ...
        select to_char(available_date,'Month DD, YYYY') available_date
         ...
         where product_id = :product_id
        "]
        if { [parameter::get -package_id [ec_id] -parameter AllowPreOrdersP -default 0 -boolean] } {
            set r "
            <form method=post action=\"[ec_url]$form_action\">
	    [export_form_vars product_id]
            [export_form_vars access_token expires_in app_id]
	    [ec_decode $order_id "" "" [export_form_vars order_id]]
	    $color_widget $size_widget $style_widget
...
ad_proc ec_order_summary_for_customer { 
	...
	append items_ul "<li>Quantity $quantity: Stock No $sku, $product_name; $options$price_name: [ec_pretty_price $price_charged [parameter::get -package_id [ec_id] -parameter Currency]]"
...
ad_proc ec_price_line { 
    ...
    set currency [parameter::get -package_id [ec_id] -parameter Currency -default "BRL"]
    ...
    append price_line "[ec_pretty_price $lowest_price $currency]"

    if { [parameter::get -package_id [ec_id] -parameter UserClassApproveP -default 0 -boolean] } {
...
ad_proc ec_display_product_purchase_combinations { product_id } { display product purchase combinations } {
    ...
    for { set counter 0 } { $counter < 5 } { incr counter } {
	    ...
	    if { ![empty_string_p $item_name] } {
            append to_return "<li><a href=\"[ec_url]product?[export_vars product_id]\">${item_name}</a></li>"
            }
        }
    }
...
ad_proc ec_user_class_display { user_id { link_p "f" } } {
	...
	if { [parameter::get -package_id [ec_id] -parameter UserClassApproveP] } {
...
ad_proc ec_get_user_session_id {} { Gets the user session from cookies } {
    ns_log Notice "Running ad_proc ec_get_user_session_id"

    set headers [ns_conn headers]
    ns_log Notice "HEADER $headers"
...
ad_proc -private ec_create_new_session_if_necessary {
    ...
    ns_log Notice "Running ec_create_new_session_if_necessary"
    set token [lindex [split [lindex [split $more_url_vars_exported "&"] 0] "="] 1]
    ...
    uplevel {
	# ns_log Notice "USER SESSION ID $user_session_id"
		...
		set user_session_id [db_nextval ec_user_session_seq]
                ...
                # ns_log Notice "SESSION ID ****** $user_session_id"
		
                set ip_address      [ns_conn peeraddr]
		# ns_log Notice "IP $ip_address"
		
                set http_user_agent [ecGetUserAgentHeader]
                # ns_log Notice "HTTP AGENT $http_user_agent"
		...
		if {[exists_and_not_null token] } {
		    # ns_log Notice "$more_url_vars_exported |  *****     TOKEN $token "
		    set final_page [export_vars -base $final_page {token}]
		} else {
