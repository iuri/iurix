...
ad_proc ec_order_status {  
    ...
    if { $n_shipped_items == $n_total_items } {
        if { $shippable_p } {
	    return "[_ ecommerce.All_Items_Shipped]"
	} else {
	    return "[_ ecommerce.All_Items_Fullfilled]"
	}
	...
	return "All Items Returned"
        ...
        if { $order_state eq "void" } {
            if { ![info exists reason_for_void] } {
                set reason_for_void "[_ ecommerce.none_provided]"
            }
            return "[_ ecommerce.Order_voided], [_ ecommerce.reason] $reason_for_void"
        } else {
            #return "[_ ecommerce.Order_State] $order_state [_ ecommerce.In_progress]"
	    return "[_ ecommerce.$order_state] <br> [_ ecommerce.In_progress]"
        }
    } elseif { $n_received_back_items > 0 } {
	return "[_ ecommerce.Some_Items_Returned]"
    } elseif { $n_shipped_items > 0 } {
	return "[_ ecommerce.Partial_Shipment_Made]"
    } else {
	return "[_ ecommerce.Unknown]" 
    }
}
...
ad_proc ec_stock_status { 
    ...
    Current Stock stauts are:

    "o" Out of Stock
    "q" Usually Ships Within 24 Hours
    "m" Usually Ships Within 2-3 Days
    "s" Usually Ships Within 2-4 Weeks
    "i" In Stock
