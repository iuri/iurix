ad_library {
    ...
    @cvs-id $Id: resource-procs.tcl,v 1.6 2008/09/04 12:24:06 torbenb Exp $
}
...
ad_proc -private ecommerce::resource::make_product_images {
    ...
    ns_log Notice "Running ad_proc make_product_images ..."
    ns_log Notice "
    $product_id \n
    $tmp_filename \n
    $product_name \n
    ...
    if [catch {
	set file_extension ".[lindex [exec $convert -identify $tmp_filename $path] 1]"
    } errmsg ] {

	
	ns_log Notice "ERROR IDENTIFYING FILE EXTENSION $errmsg"
	set file_extension $file_extension
    }
    
    #set file_extension_actual [file extension $tmp_filename]
    set file_extension_actual $file_extension
    
    set file_ext_lower [string tolower $file_extension_actual]
    if { ![string match {.[jg][pi][gf][pg]} $file_ext_lower] } {
	ns_log Notice  "DONT MATCH"
        set file_ext_lower [string tolower $file_extension]
    }
    if { $file_ext_lower eq ".jpg" || $file_ext_lower eq ".jpeg" || $file_ext_lower eq ".gif" || $file_ext_lower eq ".png" } {
	ns_log Notice  "EUQAL $file_ext_lower"
	...
	#ns_log Notice "PERMFILE $perm_filename"
	...
	#ns_log Notice "AFTER ASSIGING SUCCESS"
...
ad_proc -private ecommerce::resource::resize_image {
    ...
    #ns_log Notice "Running TCL ad_proc resize_image"
    ...
    if { $type eq "Thumbnail" || $type eq "ProductImage" } {
        ...
        # get dimensions
	set use_both_param_dimensions [parameter::get -package_id [ec_id] -parameter "${type}SizeOuterlimits"]
	
        set width_is_blank [catch {set width [parameter::get -package_id [ec_id] -parameter "${type}Width"]} ]

	set height_is_blank [catch {set height [parameter::get -package_id [ec_id] -parameter "${type}Height"]} ]
...
ad_proc -private ecommerce::resource::image_info {
    ...
    # ns_log Notice "Running ad_proc image_info "
	...
	set full_filenames_list [glob -nocomplain -- [file join $dir_path "product${type}.\[jJgG\]\[pPgG\]\[gGfF\]"]]
	...
	set type ""
        set full_filenames_list [glob -nocomplain -- [file join $dir_path "product.*"]]
    ...
    if { [file exists $image_pathname] && [file isfile $image_pathname] } {
        if { $extension eq ".jpg" || $extension eq ".jpeg" } {
            ...
            set thumbnail_size [ns_pngsize $image_pathname]
        }
