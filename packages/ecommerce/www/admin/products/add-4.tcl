...
ad_page_contract {
  ...
  @cvs-id $Id: add-4.tcl,v 1.8 2010/09/25 19:37:10 torbenb Exp $
...
db_transaction {
	...
	if {[exists_and_not_null organization_id]} {
	    application_data_link::new -this_object_id $product_id \
		-target_object_id $organization_id -relation_tag "organization__product_rel"
	}
	...
	db_dml subcategory_insert "
	    ...
	    (product_id, subcategory_id, $audit_fields)
	    values
	...
	db_dml subsubcategory_insert "
	  ...
	  (product_id, subsubcategory_id, $audit_fields)
	  values (:product_id, :subsubcategory_id, $audit_info)"
    }
	...
	if { [info exists user_class_prices($user_class_id)] } {
		...
		(product_id, user_class_id, price, $audit_fields)
		values (:product_id, :user_class_id, :uc_price, $audit_info)"
	}
    }
}
