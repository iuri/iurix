...
if {[catch {db_dml alter_ec_custom_field_values_audit_table {
    ...
    ADD (:field_identifier :column_type)
} } errmsg]} {
...
switch -exact $database_type {
	...
	db_with_handle db {
	    ...
	    for {set i 0} {$i < [db_string select_fields { SELECT COUNT(*) FROM ec_custom_product_field_values} ] } {incr i} {
		lappend trigger_column_list [lindex [db_list select_name { SELECT name FROM ec_custom_product_field_values } ] $i]
	    }
	}
	...
	db_with_handle db {
	    set trigger_column_list [list]
	    for {set i 0} {$i < [db_string select_fields { SELECT COUNT(*) FROM ec_custom_product_field_values} ] } {incr i} {
		lappend trigger_column_list [lindex [db_list select_name { SELECT field_name FROM ec_custom_product_fields } ] $i]
	    }
	}
