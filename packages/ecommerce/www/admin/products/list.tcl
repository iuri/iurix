...
ad_page_contract {
    ...
    @cvs-id $Id: list.tcl,v 1.7 2008/09/04 12:24:06 torbenb Exp $
    @author ported by Jerry Asher (jerry@theashergroup.com)
} {
    category_id:integer,notnull,optional
    order_by:optional
    {how_many:naturalnum {[parameter::get -package_id [ec_id] -parameter ProductsToDisplayPerPage]}}
    {start_row:naturalnum "0"}
    organization_id:naturalnum,optional
}
...
if {[info exists organization_id]} {
    #set product_ids [application_data_link::get_linked -from_object_id $organization_id -to_object_type "ec_product" -relation_tag "organization_product_rel"]
    set product_ids [application_data_link::get -object_id $organization_id -relation_tag "organization_product_rel"]
    ns_log Notice "product_ids **** $product_ids"
    if {[llength $product_ids] > 0} {
	set product_ids [join $product_ids ","]
	set where_clauses "AND p.product_id IN ($product_ids)" 
    }
    
}


ns_log Notice "WHERE $where_clauses"




db_foreach product_select "SELECT ep.product_id, ep.product_name, ep.available_date, ep.last_modified, count(distinct eir.item_id) as n_items_ordered, count(distinct epc.comment_id) as n_comments FROM ec_products ep, ec_items_reportable eir, ec_product_comments epc WHERE ep.product_id = eir.product_id(+) AND ep.product_id = epc.product_id(+) GROUP BY ep.product_id, ep.product_name, ep.available_date, ep.last_modified $category_exclusion_clause $where_clauses  $order_by_clause" {
