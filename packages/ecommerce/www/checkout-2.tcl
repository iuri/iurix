...
if {$user_id == 0} {
    ...
    ad_returnredirect [export_vars -base /register {return_url}]
    ad_script_abort
}
...
ec_create_new_session_if_necessary [export_vars address_id]
...
db_foreach get_shipping_data "
	...
	<div class=\"col-sm-2\"><input type=\"number\" name=\"quantity.[list $product_id $color_choice $size_choice $style_choice]\" style=\"width:60px;\" value=\"$quantity\" size=\"4\" maxlength=\"4\"></div>
	<div class=\"col-sm-4\">$sku <a href=\"product?product_id=$product_id\">$product_name</a>[ec_decode $options "" "" ", $options"]<br>
	[ec_price_line $product_id $user_id $offer_code]</div>
	<div class=\"col-sm-3\"></div>      
...
if { [parameter::get -package_id [ec_id] -parameter OfferTaxExemptStatusP -default 0 -boolean] } {
