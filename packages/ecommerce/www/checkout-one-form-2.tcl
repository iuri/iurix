...
ns_log Notice "SHIPPING REQUIRED $shipping_required"
...
if {[info exists claim_check] && ![empty_string_p $claim_check]} {
    ...
    ns_log Notice "AFTER GET CERTIFICATE $gift_certificate_user_id"
...
if {![info exists shipping_gateway]} {
    ...
    from ec_admin_settings"]
        ...
        set order_shipping_cost [expr { [ecds_base_shipping_price_from_order_value $bom_price 0] + $order_shipping_cost } ]
...
if { [string equal $gift_certificate_covers_cost_p "f"] } {
	...
	ns_log Notice "ADDITIONAL COUNT $additional_count_and_text"
...
db_transaction {
    ...
    if { ![info exists creditcard_number] || [empty_string_p $creditcard_number] } {
	    ...
	    SET set billing_address = :billing_address_id
	    WHERE creditcard_id = :creditcard_id
	}
    } else {
