...
# we want to present the most recent billing address, if there is one
	...
	ns_log Notice "ATTN [value_if_exists first_names] ***"
...
# where it might actually be more accurate than the alternate calculation. 
    ...
    if { $show_creditcard_form_p == "t" } {
        set customer_can_use_old_credit_cards [parameter::get -package_id [ec_id] -parameter SaveCreditCardDataP -default 0 -boolean]
    ...
    set gift_certificate_p [parameter::get -package_id [ec_id] -parameter SellGiftCertificatesP -default 0 -boolean]
