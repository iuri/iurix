...
# Move this chunk to a custom gateway package to paypal-gateway  - BEGIN
...
set expires_in [lindex $result 1]
set app_id [lindex $result 2]


# Create and Retrieve paypal order
ns_log Notice "ACCESS TOKEN $access_token"
if { [exists_and_not_null access_token] } {
    # Move this chunk to a custom gateway package to paypal-gateway  - BEGIN
    set base_url "https://api.sandbox.paypal.com"
	...
	\"purchase_units\": \[
	  \{
	    \"amount\": \{
	      \"currency_code\": \"BRL\",
	      \"value\": \"$order_total_price_pre_gift_certificate\"
	    \}
	  \}
	\]
    \}"
	...
	#    package require json
	set result [json::json2dict $result]
	set result [list [dict get $result id] [dict get $result status] [dict get $result links]]
    }
