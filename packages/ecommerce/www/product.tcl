...
ns_log Notice "Running TCL script product.tcl"
...
ec_create_new_session_if_necessary [export_vars {product_id offer_code}] cookies_are_not_required
...
if { [string compare $user_session_id "0"] != 0 } {
    set user_session_id [lindex $user_session_id 0]
...
set allow_pre_orders_p [parameter::get -package_id [ec_id] -parameter AllowPreOrdersP]
...
# query that follows.
ns_log Notice "TEMPLATE ID $template_id"
...
if { ![empty_string_p $template_id] } {
    ...
    ns_log Notice "TEMPLATE LIST $template_list"
...
ns_log Notice "AFTER TEMPLATE $template"
...
ns_log Notice "AFTER PRODUCT CODE $product_code"

set product_code_output [template::adp_eval product_code]

ns_log Notice "AFTER PRODUCT CODE OUTPUT"
