ad_page_contract {
    ...
    exists_product_id_p {
	if {![regexp {^([0-9]+)$} $product_id]} {
	    set return_url [export_vars -base "/items/item-add" {product_id}]
	    set product_id [lindex [split $product_id "-"] 0]
	}
    }
}
...
ec_create_new_session_if_necessary [export_vars -base "" {product_id return_url}]
...
ns_log Notice "ORDER ID $order_id"
...
if { [value_if_exists order_id] < 1 || [ad_var_type_check_number_p $order_id] == 0 } {
    ns_log Notice "INSIDE IF statement"
	...
	ad_returnredirect [export_vars -base "product" {product_id}]
        ad_script_abort
    }
}
...
if {$return_url eq "/itens/search"} {
    ad_returnredirect [export_vars -base "https://www.evex.co/itens/search" {{msg addtocart}}]
}

if {$return_url ne ""} {
    ad_returnredirect [export_vars -base $return_url {{msg addtocart} total product_id}]
} else {
    ad_returnredirect [export_vars -base "/ecommerce/shopping-cart"]
}
ad_script_abort
