...
ns_log Notice "USERSESSIONID $user_session_id"
ec_create_new_session_if_necessary [export_vars -base "" {product_id}] ]
...
if {$user_id ne ""} {
    ...
    ns_log Notice "$content_type"
    
    set type $content_type
    set return_url [export_vars -base "" {type}]
    
    if {$content_type eq "ee_service" } {
	set type "s"
    } elseif {$content_type eq "ee_venue" } {
	set type "v"
    }
    
    
    
   ad_returnredirect "https://www.evex.co/welcome/search_items"
} else {

    ad_returnredirect [export_vars -base "shopping-cart" {product_id}]
}
