...
db_multirow -extend { line_subtotal } in_cart get_products_in_cart "
      select p.product_id, p.product_name, p.one_line_description, p.no_shipping_avail_p, p.shipping, p.shipping_additonal, p.weight, p.product_id, p.sku, count(*) as quantity, u.offer_code, i.color_choice, i.size_choice, i.style_choice, '' as price 
      from ec_orders o
	  ...
	  set item_id [application_data_link::get_linked -from_object_id $product_id -to_object_type "content_item" -relation_tag "item_product_rel"]    
	  if {$item_id > 0 } {
	      set product_name [content::item::get_title -item_id $item_id]
	  }
          set line_subtotal "$quantity"
	  lappend product_ids $product_id
      }
...
#    template::multirow set in_cart $i price "[lindex $lowest_price_and_price_name 1]:&nbsp;&nbsp;[ec_pretty_price [lindex $lowest_price_and_price_name 0] $currency]"
    template::multirow set in_cart $i price "[ec_pretty_price [lindex $lowest_price_and_price_name 0] $currency]"
...
## Added proposal chunk  Iuri Sampaio (iuri@iurix.com)
set event_id [ad_get_cookie event_session_id]
ns_log Notice "COOKIE SISSION SHOPPIONG CART EVENTID $event_id"

if {![db_0or1row event_exists_p {
    select cal_item_id FROM cal_items where cal_item_id = :event_id
}] } {
    
    # Unset event_session_id 
    ad_unset_cookie event_session_id
    set event_id ""
}
