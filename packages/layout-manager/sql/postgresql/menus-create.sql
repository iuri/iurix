...
SELECT acs_object_type__create_type (
	...
	'layout_menus',		-- table_name
	'menu_id',		-- id_column
	'layout-manager',	-- package_name
	'f',			-- abstract_p
	null,			-- type_extension_table
	'layout_menu.name'	-- name_method
);

insert into acs_object_type_tables (object_type, table_name, id_column)
values ('layout_menu', 'layout_menus', 'menu_id');
...
CREATE TABLE layout_menus (
	menu_id 		integer
				constraint layout_menu_id_pk
				primary key
				constraint layout_menu_id_fk
	...
	enabled_p		char(1) default('t')
				constraint layout_menus_enabled_ck
	...
	constraint layout_menus_label_un
	unique(label)
);

create or replace function layout_menu__new (integer, varchar, timestamptz, integer, varchar, integer,
varchar, integer, varchar, varchar, varchar, integer, integer, varchar) returns integer as '
declare
...
create or replace function layout_menu__delete (integer) returns integer as '
DECLARE
	p_menu_id	alias for $1;
BEGIN
	-- Erase the layout_menus item associated with the id
	delete from  	layout_menus
...
create or replace function layout_menu__del_module (varchar) returns integer as '
DECLARE
...
BEGIN
	...
	loop
		...
		PERFORM layout_menu__delete(row.menu_id);
...
create or replace function layout_menu__name (integer) returns varchar as '
DECLARE
	p_menu_id	alias for $1;
	v_name		layout_menus.name%TYPE;
BEGIN
	select	name into v_name from layout_menus
	where	menu_id = p_menu_id;
...
create or replace function layout_new_menu (varchar, varchar, varchar, varchar, integer, varchar, varchar) 
returns integer as '
declare
...
begin
	...
	from	layout_menus m where m.label = p_parent_menu_label;

	v_menu_id := layout_menu__new (
		null,					-- p_menu_id
		''layout_menu'',			-- object_type
		now(),					-- creation_date
		null,					-- creation_user
...
create or replace function layout_new_menu_perms (varchar, varchar) 
returns integer as '
declare
	p_label			alias for $1;
	p_group			alias for $2;
...
begin
	select	menu_id into v_menu_id
	from	layout_menus where label = p_label;
...
begin
	...
	PERFORM acs_permission__grant_permission(v_main_menu, v_public, ''read'');
	PERFORM acs_permission__grant_permission(v_main_menu, v_users, ''read'');
