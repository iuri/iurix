...
ad_proc -public pa_image_width_height {
    ...
    set identify_string [exec [parameter::get -parameter ImageMagickPath -default "/usr/bin"]/identify $filename]
...
ad_proc -public pa_make_new_image {
    ...
    exec [parameter::get -parameter ImageMagickPath -default "/usr/bin"]/convert -geometry $geometry -interlace None -sharpen 1x2 $base_image $new_image
...
ad_proc -public  pa_file_info {
    ...
    if { [ catch {set out [exec [parameter::get -parameter ImageMagickPath -default "/usr/bin"]/identify -format "%w %h %m %k %q %#" $file]} errMsg]} { 
        return -code error $errMsg
    }            
    ...
    foreach image_file $image_files {
        ...
        if {$base_mime eq ""} { 
            ...
            if {$base_colors ne "" && $base_colors < 257} { 
                # convert it to a png
                if {[catch {exec [parameter::get -parameter ImageMagickPath -default "/usr/bin"]/convert $image_file PNG:$new_image.png} errMsg]} { 
                ...
                # convert it to a jpg
                if {[catch {exec [parameter::get -parameter ImageMagickPath -default "/usr/bin"]/convert $image_file JPG:$new_image.jpg} errMsg]} { 
...
ad_proc pa_rotate {id rotation} {
    ...
    if {$rotation ne "" && $rotation ne "0" } { 
        ...
        db_foreach get_image_files {} {
            ...
            if {[catch {exec [parameter::get -parameter ImageMagickPath -default "/usr/bin"]/convert -rotate $rotation [cr_fs_path]$filename [cr_fs_path]${filename}.new } errMsg]} { 
