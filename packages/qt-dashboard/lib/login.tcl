...
if { [auth::UseEmailForLoginP] } {
	...
	-form [list [list email:text($username_widget),nospell \
			 [list label ""] \
			 {html {style "width:100%; border-radius:5px" placeholder "[_ acs-subsite.Email]"}}]]
	...
	-form [list [list username:text($username_widget),nospell \
			 [list label ""] \
			 {html {style "width:100%; border-radius:5px;" placeholder "[_ acs-subsite.Username]"}}]]
...
ad_form -extend -name login -form {
    {password:text(password) 
        {label ""}
	{html {style "width:100%; border-radius:5px" placeholder "[_ acs-subsite.Password]"}}
    }
}
...
ad_form -extend -name login -form {
    {ok:text(submit) {label "LOGIN"} {value "2"} {html {class "form-control btn btn-login"} }}
