...
RETURNS int4 AS ' 
       ...
       BEGIN 
		...
		PERFORM videos__delete(row.video_id);
...
DROP FUNCTION videos__delete(int4);
DROP FUNCTION videos__new (int4, "varchar", "varchar", int4, timestamptz, int4, "varchar", "varchar", "varchar", int4);

PERFORM acs_rel_type__drop_type('videos_image_thumbnail_rel','t');
PERFORM acs_rel_type__drop_role('videos_object');
PERFORM acs_object_type__drop_type('videos_image_thumbnail_rel','t');
...
PERFORM content_folder__unregister_content_type(-100,'videos_object','t');
PERFORM content_folder__unregister_content_type(-100,'videos_image_object','t');
